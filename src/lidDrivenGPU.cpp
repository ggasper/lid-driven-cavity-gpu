...
class CudaSparseSolver {
    ...
    scal_t *d_b;
    scal_t *d_x;
    const Eigen::SparseMatrix<scal_t, Eigen::RowMajor> *mat;
    public:    

    void set_matrix(const Eigen::SparseMatrix<scal_t, Eigen::RowMajor> &matrix) {
        A_rows = matrix.rows();
        A_nnz = matrix.nonZeros();
        const scal_t *hA_values = matrix.valuePtr();
        const int *hA_columns = matrix.innerIndexPtr();
        const int *hA_csrOffsets = matrix.outerIndexPtr();
        // Allocate space on device
        cudaMalloc((void**) &dA_values, A_nnz * sizeof(scal_t));
        cudaMalloc((void**) &dA_columns, A_nnz * sizeof(int));
        cudaMalloc((void**) &dA_csrOffsets, (A_rows + 1) * sizeof(int));

        // Copy from host to device
        cudaMemcpy(dA_values, hA_values, A_nnz * sizeof(scal_t), cudaMemcpyHostToDevice);
        cudaMemcpy(dA_columns, hA_columns, A_nnz * sizeof(int), cudaMemcpyHostToDevice);
        cudaMemcpy(dA_csrOffsets, hA_csrOffsets, (A_rows + 1) * sizeof(int), cudaMemcpyHostToDevice);
    }
    // Eigen::VectorXd solve(Eigen::VectorXd &b) {
    //     scal_t *d_b;

    // }
   CudaSparseSolver(const Eigen::SparseMatrix<scal_t, Eigen::RowMajor> &A, cusparseHandle_t cusparse_handle, cusolverSpHandle_t cusolver_handle): cusparse_handle{cusparse_handle}, cusolver_handle{cusolver_handle} {
        mat = &A;
        // Initialize matrix on the device
        set_matrix(A);
        ...
        // cusolverSpXcsrqrAnalysisBatched(cusolver_handle, A_rows, A_rows, A_nnz, descrA, dA_csrOffsets, dA_columns, info);

        // cusolverSpDcsrqrBufferInfoBatched(cusolver_handle, A_rows, A_rows, A_nnz, descrA, dA_values, dA_csrOffsets, dA_columns, 1, info, &internalDataInBytes, &workspaceInBytes);
        // cudaMalloc(&pBuffer, workspaceInBytes);
        cudaMalloc((void**)&d_b, sizeof(scal_t) * A_rows);
        cudaMalloc((void**)&d_x, sizeof(scal_t) * A_rows);
    }
    Eigen::VectorXd solve(const Eigen::VectorXd &b) {
        //cudaMemcpy(d_b, b.data(), sizeof(scal_t) * A_rows, cudaMemcpyHostToDevice);
        // cusolverSpDcsrqrsvBatched(cusolver_handle, A_rows, A_rows, A_nnz, descrA, dA_values, dA_csrOffsets, dA_columns, d_b, d_x, 1, info, pBuffer);
        int singularity = -1;
        //cusolverSpDcsrlsvqr(cusolver_handle, A_rows, A_nnz, descrA, dA_values, dA_csrOffsets, dA_columns, d_b, EPS, 3, d_x, &singularity);
        Eigen::VectorXd res(mat->rows());
        cusolverSpDcsrlsvluHost(cusolver_handle, mat->rows(), mat->nonZeros(), descrA, mat->valuePtr(), mat->outerIndexPtr(), mat->innerIndexPtr(), b.data(), EPS, 1, res.data(), &singularity);
        //std::cout << "Singularity: " << singularity << std::endl;
        // cudaMemcpy(res.data(), d_x, sizeof(scal_t) * A_rows, cudaMemcpyDeviceToHost);
        return res;
    }
    ~CudaSparseSolver() {
        //cudaFree(pBuffer);
...
class LidDriven {
  public:
    ...
    explicit LidDriven(XML &param_file) {
        ...
        rhs_p[N] = 0.0;
        M_p.makeCompressed();
        double preprocessing_time = static_cast<double>(clock() - preprocessing_start)/CLOCKS_PER_SEC;
        clock_t lu_start = clock();
        std::cout << "Problem size: " << M_p.rows() << " " << M_p.cols() << ", nonzeros: " << M_p.nonZeros() << std::endl;
        ...
        //CudaSparseSolver solver_cuda{M_p, cusparse_handle, cusolver_handle};
        scal_t t = 0;
        ...
        while (t < end_time) {
            ...
            CudaSparseSolver solver_cuda{M_u, cusparse_handle, cusolver_handle};
            ...
            Eigen::VectorXd solution = solver_cuda.solve(rhs_u); 
            ...
            for(int p_iter = 0; p_iter < max_p_iter; ++p_iter) {
                ...
                PV_construction_time += static_cast<double>(clock() - PV_construction_start)/CLOCKS_PER_SEC;
