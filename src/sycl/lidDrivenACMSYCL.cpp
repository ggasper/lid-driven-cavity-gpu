...
// #pragma omp parallel for default(none) schedule(static) \
    shared(interior, u_partial, u, dt, op_e_v, Re)
            ...
            for (p_iter = 0; p_iter < max_p_iter; ++p_iter) {
                ...
                max_norm = 0;
// #pragma omp parallel for default(none) schedule(static) \
    shared(interior, u, u_partial, dt, op_e_s, p, p_old) reduction(max : max_norm)
                for (int _ = 0; _ < interior.size(); ++_) {
                    int i = interior[_];
                ...
                max_div = 0;
// #pragma omp parallel for default(none) schedule(static) \
    shared(interior, op_e_v, u, p, p_new, p_old, C, dt, op_e_s, Re) reduction(max : max_div)
                for (int _ = 0; _ < interior.size(); ++_) {
                    int i = interior[_];
